...
params_wms <- function(){
  wms <- list(metric = list(name = "Métrique",
              ...
              #                  url = 'https://data.geopf.fr/wmts?REQUEST=GetTile&SERVICE=WMTS&VERSION=1.0.0&STYLE={style}&TILEMATRIXSET=PM&FORMAT={format}&LAYER=ELEVATION.ELEVATIONGRIDCOVERAGE.HIGHRES&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}',
              #                  language = "",
              #                  service = "WMS",
              #                  version = "",
              #                  sld_version = "",
